...
void display(){
	...
	for (r=0; r<level[f][0]; r++){
        for (c= 0; c<level[f][1]; c++)
			if (seat[w][f][r][c]==id)	printf("%2c",'2');					
			else if (seat[w][f][r][c]!='0') printf("%2c",'1');
			else printf("%2c",'0');		
		printf("\n");
		}					   		
} 
...
void reset(){
	int w,f,r,c;	
	FILE *fp1,*fp2;	
	int new_nfloor,new_level[NFLOOR][2]={0};
	fp1=fopen("seat.dat","r");
	fp2=fopen("config.dat","r");
	if(fp1==NULL){
		printf("File  \"seat.dat\" create failed.Command cannot be implemented.\n");
		return;
	}
	if(fp2==NULL){
		printf("File \"config.dat\" open failed.Command cannot be implemented.\n");
		return;
	}
	fscanf(fp2,"%4d",&nfloor);
	for(int i=0;i<nfloor;i++){
	...
	if(fp2==NULL){
		printf("File \"config.dat\" create failed.Command cannot be implemented.\n");
		return;
	}
	printf("Enter the new number of floors.\n");
	scanf("%d",&new_nfloor);
	fprintf(fp2,"%4d",new_nfloor);
	for(int i=0;i<new_nfloor;i++){
		printf("Enter the number of rows and columns for this layer.\n");
		scanf("%d %d",&new_level[i][0],&new_level[i][1]);
		fprintf(fp2,"%4d%4d",new_level[i][0],new_level[i][1]);
	}
	for (w=0; w< NWEEK; w++)
         for (f=0; f<NFLOOR; f++)
               for (r=0; r<NROW; r++)
                     for (c= 0; c<NCOLUMN; c++){
                        if(f<new_nfloor&&r<new_level[f][0]&&c<new_level[f][1]){
                            if(seat[w][f][r][c]=='*') seat[w][f][r][c]='0';
                        }							
						else{
                            if(seat[w][f][r][c]!='*') seat[w][f][r][c]='*';
                        }							
						fputc(seat[w][f][r][c], fp1);						  
					} 						      
    fclose(fp1);
